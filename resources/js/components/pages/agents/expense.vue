       ...
       methods: {
           async showaddExpenseModal(){
                ...
                const [resTypes, resSub] = await Promise.all([
                    ...
                    this.callApi('get', '/agent/expenses/expenses_types'),
                    this.callApi('get', '/agent/expenses/expense_sub_types')
		   ...
		   async addExpense(){
				...
				const res = await this.callApi('post', '/agent/expenses/create_expense', this.data);
				if(res.status == 201){
		    ...
		    async editExpense(){
			   ...
			   if(this.editData.type.trim() == ''){
				...
				const res = await this.callApi('post', '/agent/expenses/edit_expense', this.editData);
				if(res.status == 200){
			...
			async deleteAction(){
				...
				const res = await this.callApi('post', '/agent/expenses/delete_expense', this.deleteData);
				if(res.status == 200){
		...
		async created() {
            ...
            const res = await this.callApi('get', '/agent/expenses/all_expenses');
